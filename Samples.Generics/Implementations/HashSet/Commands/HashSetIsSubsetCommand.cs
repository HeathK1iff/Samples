...
internal class HashSetIsSubsetCommand : GenericCommandBase<HashSet<int>>
{
    public HashSetIsSubsetCommand(HashSet<int> hashSet, IConsole console) : base(hashSet, console)
    {
    }

    protected override void DoExecute(IConsole console, HashSet<int> hashSet)
    {
        console.WriteLine("Method: bool IsSubset(int[])");

        bool isSuperSet = hashSet.IsSubsetOf(console.AskNumberSet());
